...
@external
func claim{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(tokenId: Uint256) {
    alloc_locals;
...
func _claim_resources_iter{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    if (_check == 0) {
        return _claim_resources_iter(
            tokenId, caller, building_list_len - 5, building_list + 5, current_block
        );
    }
    ...
    let (gold_addr) = IModuleController.get_external_contract_address(
    ...
    let (
        local building_fixed_len: felt, local building_fixed: felt*
    ) = IM03Buildings.view_fixed_data_claim(m03_addr, building_list[1], 1);
    ...
    if (is_inf == 1) {
        ...
        let (local erc20_addr) = IModuleController.get_external_contract_address(
            controller, ExternalContractsIds.Gold
        );
        ...
        IM03Buildings.update_building_claimed_data(
            m03_addr, tokenId, building_list[0], nb_recharges - nb_blocks, current_block
        );
        ...
        let (local erc1155_addr) = IModuleController.get_external_contract_address(
            controller, ExternalContractsIds.Resources
        );
        ...
        IM03Buildings.update_building_claimed_data(
            m03_addr, tokenId, building_list[0], 0, current_block
        );
...
func harvest{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    // Fetch fixed data
    let (building_data: BuildingFixedData) = IM03Buildings.view_fixed_data(
        m03_addr, res_type, level
    );
...
func _has_resources{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    local check = is_le(costs[1] * multiplier, felt_balance);
...
func _burn_resources{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    local check = is_in_range(felt_balance, 0, RESOURCES_START);
