...
func _initialize_global_data_iter{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    type_len: felt,
    type: felt*,
    level: felt,
    building_cost: felt*,
...
func build{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    tokenId: Uint256, building_type_id: felt, pos_start: felt
) {
    alloc_locals;
    let (caller) = get_caller_address();
    ...
    let (has_resources) = IM02Resources.has_resources(
        m02_addr, caller, erc1155_addr, costs_len, costs, 1
    );
...
func upgrade{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    // Fetch fixed building data
    let (building_data: BuildingFixedData) = building_global_data.read(
        building_type_id, current_level + 1
    );
...
func recharge_building{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    // Resources
    let (has_resources) = IM02Resources.has_resources(
        m02_addr, caller, erc1155_addr, costs_len, costs, nb_days
    );
    with_attr error_message(
            "M03_Buildings: caller has not enough resources to recharge building.") {
        assert has_resources = 1;
    }
...
func _get_all_building_data_iter{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    let (type_id) = _building_data.read(
        token_id=token_id, building_id=counter, storage_index=BuildingData.type_id
    );
    ...
    let (pos_start) = _building_data.read(
        token_id=token_id, building_id=counter, storage_index=BuildingData.pos
    );
    let (last_claim) = _building_data.read(
        token_id=token_id, building_id=counter, storage_index=BuildingData.last_claim
    );
    let (recharges) = _building_data.read(
        token_id=token_id, building_id=counter, storage_index=BuildingData.recharged
    );
